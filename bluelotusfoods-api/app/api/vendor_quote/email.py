...
async def send_quote_email(quote_id: int):
    """Send email confirmation for a vendor quote"""
    conn = get_connection()
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            logger.info(f"📧 Vendor email: {quote_row['contact_email']}, Vendor name: {quote_row['vendor_name']}")
            logger.info(f"🌐 Email service URL: {settings.email_service_url}/email/vendor-quote")
            logger.info(f"📦 Quote data keys: {list(quote_data.keys())}")
            
            async with httpx.AsyncClient() as client:
                logger.info(f"⏳ Sending request to email service...")
                response = await client.post(
                    f"{settings.email_service_url}/email/vendor-quote",
                    json=email_payload,
                    timeout=30.0
                )
                ...
                if has_email_log_table:
                    cur.execute(
                        ...
                        (quote_id, quote_row['contact_email'], 'sent' if email_result['success'] else 'failed')
                    )
                    conn.commit()
